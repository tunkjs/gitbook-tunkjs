...
class userAdmin {
	...
	async getUserDetails(id){
	    ...
	    // 由于details数据不会被其他模块或视图组件复用，不必作为状态去维护，因此未在构造器中定义为状态字段

        // 由于未定义detials为状态字段，return的details字段不会产生状态变更，也不会触发状态变更钩子
        // 那么有什么用呢？发起action执行会返回action执行结果

        // 假设其他模块或视图组件调起当前模块的 getUserDetails，可以获得return的内容
